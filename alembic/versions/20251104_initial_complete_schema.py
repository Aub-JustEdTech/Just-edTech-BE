...
down_revision = '20251103_000000'
branch_labels = None
depends_on = None


def upgrade():
    # Drop existing enum types if they exist (in case of previous failed migrations)
    # This ensures we start with a clean slate
    conn = op.get_bind()
    conn.execute(sa.text("DROP TYPE IF EXISTS processingstatus CASCADE"))
    conn.execute(sa.text("DROP TYPE IF EXISTS jobstatus CASCADE"))
    conn.execute(sa.text("DROP TYPE IF EXISTS processingstage CASCADE"))
    conn.execute(sa.text("DROP TYPE IF EXISTS stagestatus CASCADE"))
    conn.execute(sa.text("DROP TYPE IF EXISTS batchstatus CASCADE"))
    
    # Create custom enum types with lowercase values to match Python enum .value attributes
